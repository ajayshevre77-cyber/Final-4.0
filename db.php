...
function get_db() {
    ...
    try {
        ...
        // Ensure assignment_submissions can hold multiple files
        try {
            $pdo->exec("ALTER TABLE assignment_submissions MODIFY file_name TEXT");
            $pdo->exec("ALTER TABLE assignment_submissions MODIFY file_path TEXT");
            $pdo->exec("ALTER TABLE assignment_submissions MODIFY file_type TEXT");
            $pdo->exec("ALTER TABLE assignment_submissions MODIFY file_size TEXT");
        } catch (Exception $ex) {}
...
function save_db($data) {
    ...
    try {
        ...
        if (isset($data['assignment_grievances'])) {
            ...
            $stmt = $pdo->prepare("INSERT INTO assignment_grievances (id, subject_assignment_id, student_id, student_name, issue_type, description, screenshot, status, reply) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
